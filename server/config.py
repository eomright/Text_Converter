...
# ── 중단 대비 ────────────────────────────────────────────────────────
# 50~80분 강의를 large-v3 로 변환하면 1~2시간 걸린다. 그 사이 서버가 꺼져도
# 작업이 사라지지 않게 결과를 조금씩 저장하고, 끊긴 지점부터 이어서 변환한다.
SAVE_EVERY_SEGMENTS = 8        # 이만큼 문장이 나올 때마다 저장
SAVE_EVERY_SEC = 20.0          # 또는 이만큼 시간이 지날 때마다 저장
# 서버를 켤 때 끊긴 작업을 자동으로 다시 시작할지. 끄면 기록마다 "이어서 변환" 버튼으로 시작.
# 긴 작업이 켜자마자 CPU 를 차지하지 않게 기본은 끔 (2026-09-22 사용자 결정)
AUTO_RESUME_ON_START = os.environ.get("KSTT_AUTO_RESUME", "0") == "1"
